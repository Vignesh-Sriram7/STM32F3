...
void thermal_mon(void) {
	    HAL_UART_Transmit(&huart2, (uint8_t*)"Start DHT11 read\r\n", 18, HAL_MAX_DELAY);
	    if (readDHT11(&dht11)) {
	        char buffer[50];
...
void ir_sensor_task(void)
{
    ...
    GPIO_PinState state = HAL_GPIO_ReadPin(IR_SENSOR_PORT, IR_SENSOR_PIN);

    snprintf(buffer, sizeof(buffer), "IR Sensor State: %s\r\n", (state == GPIO_PIN_SET) ? "ON" : "OFF");
    HAL_UART_Transmit(&huart2, (uint8_t*)buffer, strlen(buffer), HAL_MAX_DELAY);
}
